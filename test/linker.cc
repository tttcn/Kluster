...
int main(int argc, char *argv[]) {
  DEBUG("debug mode on\n");
  if (argc != 6) {
    printf("usage: program node_file_name edge_file_name data_num data_dim "
           "threshold\n");
    return 1;
  }
  ...
  float *node_data = (float *)mmap(NULL, node_file_size, PROT_READ, MAP_SHARED,
  ...
  if (edge_num == -1) // 阈值太高导致取得边数超过了容纳上限
  {
    ...
    // {
    //     fprintf(edge_file, "%d,%d,%f\n", edge_data[edge_id].base_id,
    //     edge_data[edge_id].query_id, edge_data[edge_id].distance);
    // }
    // fclose(edge_file);
