...
void load_data_int8(int8* data,int data_num,int data_dim){
    FILE *fd=fopen("/mnt/data/tao/tr5kw/tr05/boysenberry.int8.feature.data","rb");
    fread(data,sizeof(int8),data_num*data_dim,fd);
    printf("data loaded.\n");
    fclose(fd);
    return;
}

void load_data_int8_to_int32(int* data,int data_num,int data_dim){
    FILE *fd=fopen("/mnt/data/tao/tr5kw/tr05/boysenberry.int8.feature.data","rb");
    fread(data,sizeof(char),data_num*data_dim,fd);
    printf("data loaded.\n");
    fclose(fd);
    return;
}
...
__global__
void mul_float32(const float* device_data, int data_num, int data_dim){
    int i=blockIdx.x*data_dim;
    int j=(blockIdx.y*blockDim.x+threadIdx.x)*data_dim;
    float sum=0.0;
...
__global__
void mul_int8(const int8* device_data, int data_num, int data_dim){
    int i=blockIdx.x*data_dim;
    int j=(blockIdx.y*blockDim.x+threadIdx.x)*data_dim;
    int sum=0.0;
    for (int dim=0;dim<data_dim;++dim){
        int rest=(device_data[i+dim]-device_data[j+dim]);
        sum+=rest*rest;
    }
    // if (sum<TH) printf("bingo!");
}
...
__global__
void mul_then_add_float32(const float* device_data, int data_num, int data_dim,unsigned long long int* device_count,volatile int *device_count_array){
    ...
    if (sum<6000.0)  atomicAdd(device_count,1);
    return;
}

__global__
void mul_then_add_line_test_float32(const float* device_data, int data_num, int data_dim,unsigned long long int* device_count,float *device_dis_array,int line_idx){
    unsigned long long int i=blockIdx.x*data_dim;
    unsigned long long int j=(blockIdx.y*blockDim.x+threadIdx.x)*data_dim;
    float sum=0.0;
    for (int dim=0;dim<data_dim;++dim){
...
__global__
void mul_then_add_int8(const int8* device_data, int data_num, int data_dim){
    int i=blockIdx.x*data_dim;
    int j=(blockIdx.y*blockDim.x+threadIdx.x)*data_dim;
    int sum=0.0;
    for (int dim=0;dim<data_dim;++dim){
...
void call_cuda_float32(const float* data,int data_num,int data_dim){
    ...
    mul_then_add_float32<<<grid_dim,block_dim>>>(device_data,data_num,data_dim,device_count,device_count_array);
...
void call_cuda_int8(const int8* data,int data_num,int data_dim){
    ...
    int8 * device_data=NULL;
    unsigned long long int data_size=data_num*data_dim*sizeof(int8);
    cudaMalloc((void **) &device_data, data_size);
    cudaMemcpy(device_data,data,data_size,cudaMemcpyHostToDevice);
    printf("data copied to device.\n");

    // start_t=clock();
    // mul_int8<<<grid_dim,block_dim>>>(device_data,data_num,data_dim);
    printf("mul done.\n");
    // end_t=clock();
    // interval_t=double(end_t-start_t)/CLOCKS_PER_SEC;
    // printf("mul time: %lf\n",interval_t);


    // start_t=clock();
    mul_then_add_int8<<<grid_dim,block_dim>>>(device_data,data_num,data_dim);
    printf("mul then add done.\n");
    // end_t=clock();
    // interval_t=double(end_t-start_t)/CLOCKS_PER_SEC;
    // printf("mul then add time: %lf\n",interval_t);
...
void line_test_float32(const float* data,int data_num,int data_dim, int line_idx){
    ...
    // start_t=clock();
    mul_then_add_line_test_float32<<<grid_dim,block_dim>>>(device_data,data_num,data_dim,device_count,device_dis_array,line_idx);
    printf("gpu runnig.\n");
    for (int i=0;i<data_num;++i){
...
int main(){
    ...
    data_controller.mmap_feature("/mnt/data/tao/tr5kw/tr05/boysenberry.int8.feature.data",Dim,Num);
    
    float * data_float32=data_controller.get_feature();
    load_data_float32(data_float32,Num,Dim);
