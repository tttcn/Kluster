                   ...
                   size_t batch_len)
{
    ...
    // result_m = (float *)device.Malloc(batch_len * batch_len * sizeof(float)); // 距离结果数据
    // output_m = (coo *)device.Malloc(batch_len * batch_len * sizeof(coo)); // 输出coo数据
    ...
    // 分块计算
    // 先计算module array
    SetModule(data_m.Get(), module_m.Get(), data_num, data_dim);
    DEBUG("module set.\n");
    ...
    for (int lid = 0; cal_flag == 0 && lid < seg_num; ++lid)
    {
        for (int rid = lid; cal_flag == 0 && rid < seg_num; ++rid)
        {
            ...
            *take_num_m = 0;
            ...
            GemmBlas(lhs_m.Get(), rhs_m.Get(), result_m.Get(), batch_len, batch_len, data_dim);
#ifdef DEBUG_
            gemmtest(data_m, result_m, batch_len, lid, rid);
#endif
            ModuleTake(result_m.Get(), module_base.Get(), module_query.Get(), take_num_m.Get(), output_m.Get(), batch_len, batch_len, threshold);
#ifdef DEBUG_
            ...
            for (int edge_id = 0; cal_flag == 0 && edge_id < *take_num_m.Get(); ++edge_id)
            {
